...
export const mailToSuppliers = (orderArray, supplier) => {
  const filteredOrderRowsArray = orderArray
    .filter((orderItem) => orderItem.itemInfo.item.Supplier === supplier.name);

	const sortedOrderRowsArray = filteredOrderRowsArray.sort((a, b) => (a.itemInfo.item.Code > b.itemInfo.item.Code ? 1 : -1));

    const formattedOrderRowsArray = sortedOrderRowsArray.map((orderItem) => {
  ...
  const orderString = formattedOrderRowsArray.join("%0D%0A");
